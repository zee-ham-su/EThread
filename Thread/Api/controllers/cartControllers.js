...
const CartController = {
  ...
  deleteCartItem: async (req, res) => {
    ...
    // Find the deleted item from the cart items array
    const deletedItemIndex = cart.items.findIndex(item => item._id.toString() === itemId);
    if (deletedItemIndex === -1) {
      return res.status(404).json({ message: 'Item not found in cart' });
    }
    const deletedItem = cart.items[deletedItemIndex];

    // Remove the item from the cart items array
    cart.items.splice(deletedItemIndex, 1);
    ...
    console.log('Cart updated after item deletion:', cart);
